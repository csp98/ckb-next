...
static const Key K55Zones[] = {
    {0, "Zone 1", "esc", 46, 45, ZS, true, false},
    {0, "Zone 2", "grave", 140, 45, ZS, true, false},
    {0, "Zone 3", "tab", 233, 45, ZS, true, false},
};
...
static QHash<QString, Key> getMap(KeyMap::Model model, KeyMap::Layout layout){
    ...
    case KeyMap::K55:{
        ...
        for(size_t cnt = 0; cnt < K55_ZONES; cnt++){
            QString* name = new QString("zone");
            name->append(QString::number(cnt));
            map[name->toLatin1().data()] = K55Zones[cnt];
        }
        break;
    }
    case KeyMap::STRAFE:{
